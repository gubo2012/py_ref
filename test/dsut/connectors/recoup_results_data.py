...
Pulls Recoup and Result sets for training data
...
client_prefix = 'S8'
review_tbl_name = f'zz_{client_name}_recoup_review_data'
master_tbl_name = f'zz_{client_name}_master_format_data'
...
select * from (
    ...
    where 1=1
    and QueryID like '{client_prefix}0250I%'
    and ReviewType in ('C', 'D')
    and c.BillFromDate >= '2017-01-01' ) a
...
def result_set_worker(job: pd.DataFrame, tbl_name: str = None) -> None:
    """
    ...
    if job.shape[0] != df.shape[0]:
        print(f'- Recoup # of claims: {job.shape[0]:,} \nResult set # of claims: {df.shape[0]:,}')
    else:
        print(f"Processed {job.shape[0]:,} claims for {job['queryid'].min()}")

    df.to_sql(tbl_name, njtpl_eng, if_exists='append', index=False)
...
if __name__ == '__main__':
    ...
    for sub_df in pd.read_sql(recoup_query, pi_eng, chunksize=1_000):
        ...
        sub_df.to_sql(review_tbl_name, njtpl_eng, if_exists='append', index=False)
        num += sub_df.shape[0]
        print(f'{num:,} records written down.')

    # retrieve the data from result sets in batch style
    with ThreadPoolExecutor(4) as ex:
        for jobs_df in pd.read_sql(f'select * from {review_tbl_name}', njtpl_eng, chunksize=10_000):
            for _, sub_df in jobs_df.groupby('queryid'):
                ex.submit(result_set_worker, sub_df, master_tbl_name)
